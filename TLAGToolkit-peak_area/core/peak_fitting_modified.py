...
class Peak_fitter:
    ...
    def get_config_dict_variables(self, config_dict):
        ...
        self.models_defined = config_dict['xrd']['peaks']
    ...
    def fit_model(self, model, x_spaced, y_corrected, params, interval, midpoint, noise, ratio_new_points = 1, plot_results = False, i = 0):
        ...
        # plt.plot(x_spaced, background)
        # plt.show()
        # print(background[ind])
    ...
    def continuous_fitting(self, fitting_direction="forward", plot_mse=False):
        ...
        if fitting_direction == "forward":
            ...
            end_point = self.index_end+1
            step = self.step

        elif fitting_direction == "backward":
            initial_point = self.index_end
            end_point = self.index_start-1
            step = -self.step
        
        # Read the file
        integration_file = self.find_integration_file(self.path_scans, initial_point)
        # First XRD spectra #000 or last XRD spectra depending if we are doing the forward or backward analysis
        ...
        noise = np.std(I_noise)
        background = np.average(I_noise)
        limit = background + noise*3

        print('The noise of XRD data scan {} is I = {:.3f} a.u.'.format(initial_point, noise))
        print('Noise evaluation between 2 theta = {:.1f}-{:.1f} deg'.format(Twotheta[3050], Twotheta[3150]))

        print('The minimum Intensity for signal detection is I = {:.3f} a.u.'.format(background+noise*2))
        ...
        # for scan_index in tqdm(range(self.index_start, self.index_end, self.step), desc="Fitting: ", unit="integrations"):
            try:
                ...
                for i, group in enumerate(self.group2model.keys()):
                    ...
                    fitted_model, auc, I_max = self.fit_model(model, x_spaced, y_corrected, params, interval, midpoint, noise, ratio_new_points=self.ratio_new_points, plot_results=False, i=(scan_index - self.index_start)//5)
                ...
                peaks = []
                for key in result_params_models.keys():
                    peak, function_parameter = key.split('_')
                    if peak not in peaks:
                        peaks.append(peak)
                I = dict(zip(peaks, I.values()))
                ...
                for param_name, param in result_params_models.items():
                    ...
                    if I[group] > limit:
                        ...
                        self.fitting_data[param_name].append(param.value * 0)
        ...
        return self.mse_integrations_mean, self.fitting_data, self.XRD
    ...
    def run_fitting(self):
        ...
        mse_forward, final_parameters_forward, XRD_f = self.continuous_fitting(fitting_direction="forward")

        print("Trying to fit your models backward (indices decreasing):")
        mse_backward, final_parameters_backward, XRD_b = self.continuous_fitting(fitting_direction="backward")
        ...
        if (mse_forward < mse_backward).all():
            ...
            self.save_results(final_parameters_forward, XRD_f)
        elif (mse_forward > mse_backward).all():
            print("Backward fitting is better. Saving it.")
            self.save_results(final_parameters_backward, XRD_b)
        else:
    ...
    def save_results(self, fitting_data, XRD):
        ...
        self.config_dict["errors"] =  [{model['peak'] : float(error)} for model, error in zip(self.models_defined, self.mse_integrations_mean)]
    ...
    def initialize_fitting_data(self):
        ...
        for models in self.models_defined:
            self.fitting_data[f"{models['peak']}_AUC"] = []
    ...
    def initialize_model(self):
        ...
        for group, model_list in self.group2model.items():
            ...
            for model_id in model_list:
                model_config = self.models_defined[model_id]
                model = getattr(models, model_config['model_type'])(prefix=model_config['peak'] + '_')
                ...
                self.midpoints[model_config['peak']] = midpoint
