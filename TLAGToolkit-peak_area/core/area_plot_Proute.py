# -*- coding: utf-8 -*-
"""
ANALYSING ALBA'S DATA IN-SITU TLAG GROWTH VIA P-ROUTE
...
class data_analysis_NCD:
    def __init__(self, thickness, process, REBCO_name, time_before_jump, time_after_jump, save_figure, save_file, directory_data, plots, time1_average_grate, time2_average_grate):
        
        self.thickness = thickness
        self.process = process
        self.REBCO_name = REBCO_name
        self.time_before_jump = time_before_jump
        self.time_after_jump = time_after_jump
        ...
        self.colors_peaks = {
            ...
            f'{self.REBCO_name}(003)': 'olive',
            f'{self.REBCO_name}(103)(110)': 'purple',
            'Y2Cu2O5': 'gold',
            'Y2BaCuO5': 'cyan',
            'RE2O3': 'turquoise',
            f'{self.REBCO_name}(006)': 'magenta',
    ...
    def acquisition_time_compensation(self):
        ...
        index_jump = self.find_index_of_closest_num(self.peaks[index_dome]['time'], self.time_jump)
        self.intensity_dome_reference = self.peaks[index_dome]['I_max'][index_jump]
        self.time_acquisition_at_jump = self.peaks[index_dome]['time'][index_jump]- self.peaks[index_dome]['time'][index_jump-1]
    ...
    def find_time_jump(self):
        pressure_derivative = calculate_derivative(self.experiment_data['timestamp'], self.experiment_data['pressure'])
        index_jump = np.nanargmax(pressure_derivative) 
        self.time_jump = self.experiment_data['time'][index_jump]
        self.pressure_at_jump = self.experiment_data['pressure'][index_jump]
        self.experiment_data['mass_spectrometer'] = []

        
    def read_mass_spectrometer(self, mass_spectr_file):
    ...
    def adapt_mass_spectr_data_to_plot(self, mass_spectr_data):
        ...
        # first, we shift the data takig as a reference the time at jmupjump
        mass_spectr_data['time_clean'] = [x for x in mass_spectr_data['time_clean'] if not math.isnan(x)] 
        shift = mass_spectr_data['time_clean'][-1] - self.time_jump
        ...
        self.experiment_data['mass_spectrometer'] = mass_spectr_data
    ...
    def plot_cooling_and_pressure_jump(self):
        plt.figure(figsize=(7, 6))
        plt.subplot(2, 1, 1)
        plt.plot(self.experiment_data['time'], self.experiment_data['temperature'], color='blue')
        plt.plot(self.time_cooling, self.temp_cooling, 'o', color='red', label='start cooling')
        plt.xlabel('Time')
        plt.ylabel('Temperature')
        plt.legend(loc='upper right')

        plt.subplot(2, 1, 2)
        plt.plot(self.experiment_data['time'], self.experiment_data['pressure'], color='orange')
        plt.plot(self.time_jump, self.pressure_at_jump, 'o', color='red', label='start jump')
        plt.xlabel('Time')
        plt.ylabel('Pressure')
    ...
    def average_growth_rate(self, measurement, time_line1, time_line2):
        ...
        index_REBCO = self.find_index_peak(self.peaks, f'{self.REBCO_name}(005)')
        index_jump = self.find_index_of_closest_num(self.peaks[index_REBCO]['time'], self.time_jump)
        index_cooling = self.find_index_of_closest_num(self.peaks[index_REBCO]['time'], self.time_cooling)
        
        x_axis_plot = self.peaks[index_REBCO]['time'][index_jump-10:index_cooling-150]
        y_axis_plot = self.peaks[index_REBCO][label][index_jump-10:index_cooling-150]
        index_growth_YBCO = first_nonzero(y_axis_plot)-1
        
        index_time_line1 = self.find_index_of_closest_num(x_axis_plot, self.time_jump+time_line1)
        index_time_line2 = self.find_index_of_closest_num(x_axis_plot, self.time_jump+time_line2)
        ...
        # DOING IT AUTOMATICALLY FILTERING THE DATA
        # y_smooth = savgol_filter(y_axis_plot, window_length=50, polyorder=2)  # Adjust parameters
        # growth_rate_REBCO_peak_data = np.gradient(y_smooth, x_axis_plot) *self.thickness/max(y_smooth)
        # standard_dv = np.std(growth_rate_REBCO_peak_data[-50:-1]) 
        # n3 = find_first_bigger_from_end(growth_rate_REBCO_peak_data, standard_dv*8)
        # slope2a, intercept2a = np.polyfit(x_axis_plot[n3:-1], y_axis_plot[n3:-1], 1)
        # y_fit2a = [slope2a * xi + intercept2a for xi in x_axis_plot]
        
        # n2 = find_first_bigger_from_end(growth_rate_REBCO_peak_data, max(growth_rate_REBCO_peak_data)/3)
        # slope1a, intercept1a = np.polyfit(x_axis_plot[index_growth_YBCO:n2], y_axis_plot[index_growth_YBCO:n2], 1)
        # y_fit1a = [slope1a * xi + intercept1a for xi in x_axis_plot]
        
        # time_interception2 = (intercept2a - intercept1a) / (slope1a - slope2a)
        # y_interception2 = slope1 * time_interception2 + intercept1a
        # average_growth_rate2 = self.thickness/(time_interception2 - x_axis_plot[index_growth_YBCO])
        
        # fig2, ax1 = plt.subplots()
        # ax1.plot(x_axis_plot, y_axis_plot, alpha=0.5)
        # ax1.plot(x_axis_plot, y_smooth, linewidth=2)
        # ax1.plot(time_interception2, y_interception2, 'o', color='red')
        # ax1.plot(x_axis_plot, y_fit2a, color='red')
        # ax1.plot(x_axis_plot, y_fit1a, color='red')
        # ax1.set_ylim(min(y_axis_plot), max(y_axis_plot))
        # ax1.text(0.95, 0.05, f'average growth rate = {average_growth_rate2:.2f} nm/s', fontsize=12, bbox=dict(facecolor='white', edgecolor='black'), 
        #         transform=ax.transAxes,  verticalalignment='bottom', horizontalalignment='right')
        
        # fig3, ax2 = plt.subplots()
        # ax2.plot(x_axis_plot, growth_rate_REBCO_peak_data)
    ...
    def growth_REBCO005(self, time_line1_average_grate, time_line2_average_grate):
        ...
        index_before_jump = self.find_index_of_closest_num(self.peaks[self.index_REBCO]['time'], self.time_jump - 10)
        index_after_jump = self.find_index_of_closest_num(self.peaks[self.index_REBCO]['time'], self.time_jump + 60)
              
        # growth rate from REBCO(005) peak integrated intensity and intensity
        self.max_growth_rate_AUC = self.instantaneous_growth_rate('AUC', index_before_jump, index_after_jump)
        self.max_growth_rate_intensity = self.instantaneous_growth_rate('intensity', index_before_jump, index_after_jump)
        ...
        if 'resistance' in self.peaks[self.index_REBCO]:
            index_before_jump = self.find_index_of_closest_num(self.experiment_data['time'], self.time_jump - 10)
            index_after_jump = self.find_index_of_closest_num(self.experiment_data['time'], self.time_jump + 60)
        
            self.experiment_data['conductance'] = [1/r for r in self.experiment_data['resistance']]
            max_conductance_during_growth = max(self.experiment_data['conductance'][index_before_jump:index_after_jump])
            ...
            self.max_growth_rate_conductance = max(self.experiment_data['growth_rate'][index_before_jump:index_after_jump])
            
            fig, ax = plt.subplots()
            ax.plot(self.experiment_data['time'][index_before_jump:index_after_jump], self.experiment_data['growth_rate'][index_before_jump:index_after_jump], '-', color='black')
            ax.grid(True)
            ax.set_xlabel('Time (s)')
    ...
    def plot_2d_figure(self, time_start, time_end, yaxis, resistance_measurement):
        ...
        if self.experiment_data['mass_spectrometer']:
            mass_spectr_data = self.experiment_data['mass_spectrometer']
            index_start3 = self.find_index_of_closest_num(mass_spectr_data['time_clean'], time_start)
            index_end3 = self.find_index_of_closest_num(mass_spectr_data['time_clean'], time_end)
    ...
    def temperature_plot(self, time_start, time_end, yaxis, resistance_measurement):
    ...
    def create_txt_with_additional_info(self):
        ...
        additional_info = ["thickness", "time_jump", "intensity_dome_reference", "time_acquisition_at_jump", "time_cooling", "temp_cooling", 
    ...
    def start_analysis(self):
        ...
        self.plot_cooling_and_pressure_jump()
        self.acquisition_time_compensation()
        
        if mass_spectr_file:
            mass_spectr_data = self.read_mass_spectrometer(mass_spectr_file)
            self.adapt_mass_spectr_data_to_plot(mass_spectr_data)
        ...
        if self.time_after_jump == 'cooling':
            self.time_after_jump = self.time_cooling - self.time_jump-5

        # MAKE GRAPHS
        if self.process == 'heating':
            resistance_measurement = 'resistance'
            time_start = 0
            time_end = self.time_jump
        if self.process == 'jump':
            resistance_measurement = 'conductance'
            time_start = self.time_jump - self.time_before_jump
            time_end = self.time_jump + self.time_after_jump
        if self.process == 'cooling':
            resistance_measurement = 'resistance'
            time_start = self.time_jump - self.time_before_jump
            time_end = self.time_jump + 1000
        ...
        if self.process == 'heating':
            index_end_heating = find_start_of_decrease(self.experiment_data['temperature'][self.experiment_data['index_start']:]) + 200
            time_end = self.experiment_data['time'][index_end_heating + self.experiment_data['index_start']]
            for key in self.type_plots:
                if 'resistance' in self.experiment_data:
                    self.temperature_plot(time_start, time_end, key, resistance_measurement)
                else:
                    self.temperature_plot(time_start, time_end, key, False)
...
def main():
    ...
    directory_data = r'C:\Users\eghiara\Desktop\ALBA\NCD_Mar24\DATA\PROCESSED\YBCO\4mbar\Y12128a1'  # you should change it for the directory where your data is - from Peak area
    thickness = 400            # in nm
    process = 'heating'         # heating, jump, cooling
    REBCO_name = 'REBCO'        # YBCO, GdBCO,YGdBCO... Whatever name you gave in the excels
    time_before_jump = 10      # fsor the plot at jump (in s)
    time_after_jump = 100      # for the plot at jump (in s). If set to 'cooling' it takes the time that cooling begins
    save_figure = True        # False / True
    save_file = False          # False / True
    plots = ['AUC', 'AUC_norm']  # can be: 'I_max', 'AUC', 'I_max_norm', 'AUC_norm'
    
    # for the average growth rate calculation
    time_line1_average_grate = 10   # (in s) amount of seconds to consider for the first straight line. goes (0, time_line1_average_grate)
    time_line2_average_grate = 100   # (in s) seconds straight line goes from (time_line2_average_grate, cooling)

    # for i in range(len(process)):
    XRD_analysis = data_analysis_NCD(thickness, process, REBCO_name, time_before_jump, time_after_jump, save_figure, save_file, directory_data, plots, time_line1_average_grate, time_line2_average_grate)
