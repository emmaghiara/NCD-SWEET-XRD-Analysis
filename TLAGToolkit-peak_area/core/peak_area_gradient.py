...
class Peak_area_gradient:
    ...
    def get_config_dict_variables(self, config_dict):
        ...
        self.logs_desired = ["imgIndex", "temperature", "pressure", "resistance",  "time", "timestamp", "x", "z", "omega", "att1", "att2"]
    ...
    def peaks_area(self):
        ...
        I_noise = I_corrected[2500:2550] 
        # I_noise = I_corrected[2000:2050]  # Rocking different 2theta range
        self.noise = np.std(I_noise)
        self.background = np.average(I_noise)
        limit = self.background+self.noise*3

        print('The noise (=error) of XRD data scan #0000 is I = {:.7f} a.u.'.format(self.noise))
        print('Noise evaluation between 2 theta = {:.1f}-{:.1f} deg'.format(Twotheta[2500], Twotheta[2550]))
        # print('Noise evaluation between 2 theta = {:.1f}-{:.1f} deg'.format(Twotheta[2000], Twotheta[2050]))  # Rocking different 2theta range

        print('The minimum Intensity for signal detection is I = {:.7f} a.u.'.format(self.background+self.noise*2))
        print('The minimum Intensity for signal measure is I = {:.7f} a.u.'.format(limit))
        ...
        for scan_index in tqdm(range(self.index_start, self.index_end+1, self.step), desc="Peaks evaluation ", unit="integrations"):
            try:
                ...
                for i, peak in enumerate(self.peaks_defined):
                    ...
                    modified_interval = tuple(x + 0.0890909/2*scan_index for x in interval)   # modified to follow the 2theta shift of the peaks because of distance with the detector
                    # modified_interval = tuple(x + 0.18/2*scan_index for x in interval)   # larger modification for 4 angle, smaller for 8.
                    modified_interval = tuple(int(x) for x in modified_interval)  # I need a integer number to put in the x-axis (2theta correspond.)
                    print(Twotheta[modified_interval[0]])
                    peak_param = self.peak_calc(x, y_corrected, modified_interval, limit, peak_name)    # change when it is not a map gradient ex-situ
                    #####################

                    # peak_param = self.peak_calc(x, y_corrected, interval, limit, peak_name)
...
def main():
    ...
    peak_fitter = Peak_area_gradient(config_dict)
